...
function renderCompatibility(){const versions=getJoomlaVersions();const row=compatibilityBody.closest("table")?.querySelector("thead tr");if(row)row.innerHTML=`<th>Extension</th>${versions.map(v=>`<th>${escapeHtml(v.replace(/^J/,""))}</th>`).join("")}`;compatibilityBody.innerHTML=projects.map(p=>`<tr><td>${escapeHtml(p.name)}</td>${versions.map(v=>`<td>${(p.compatibility||[]).includes(v)?'<span class="yes">✓</span>':"—"}</td>`).join("")}</tr>`).join("");}
